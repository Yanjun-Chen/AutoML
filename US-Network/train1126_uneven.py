...
from utils.meters import ScalarMeter, flush_scalar_meters, scalar_meters
...
def dataset(train_transforms, val_transforms, test_transforms):
    ...
    if FLAGS.dataset == 'imagenet1k':
        ...
        val_set = datasets.ImageFolder(
            os.path.join(FLAGS.dataset_dir, 'valf'),
...
def data_loader(train_set, val_set, test_set):
    ...
    print("num_workers = ",FLAGS.data_loader_workers)
...
def get_meters(phase):
    ...
    if getattr(FLAGS, 'slimmable_training', False):
        meters = {}
        for width_mult in FLAGS.width_mult_list_test:
...
def profiling(model, use_cuda):
    ...
    if getattr(FLAGS, 'autoslim', False):
        ...
        for width_mult in sorted(FLAGS.width_mult_list_test, reverse=True):
            model.apply(
...
def run_one_epoch(
        epoch, loader, model, criterion, optimizer, meters, logging, phase='train',
        soft_criterion=None):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        model_pool = '/home/chenyanjun/temp/1126/slimmable_networks/Search_Space/'+str(FLAGS.search_space)
        random_list = [0, FLAGS.search_space_num-1]
            ...
            if getattr(FLAGS, 'slimmable_training', False):
                if getattr(FLAGS, 'universally_slimmable_training', False):
                    ...
                    for _ in range(getattr(FLAGS, 'num_sample_training', 2)-2):
                        widths_train.append(
                            random.uniform(min_width+1, max_width))
                    ...
                    for width_mult in widths_train:
                        ...
                        if width_mult in [max_width, min_width]:
                            for _ in range(9):
                                blocksize_list.append(width_mult)
                        else:
                            model_idx = random.randint(1, FLAGS.search_space_num-2)
                            random_list.append(model_idx)

                            f_model = open(model_pool, 'r')
                            blocksize_list = eval(f_model.readlines()[model_idx].strip())
                            
                        model.apply(
                                lambda m: setattr(m, 'blocksize_list', blocksize_list))
        ...
        if is_master() and getattr(FLAGS, 'slimmable_training', False) and batch_idx % FLAGS.print_freq ==0:
            print('\nTime {:.3f}s ({:.3f})s\tData {:.3f}s ({:.3f})s\t{}\tSearch space:{}\tepoch:{}/{}\t{}/{} '.format(
                    batch_time.val, batch_time.avg, data_time.val,data_time.avg,phase,FLAGS.search_space, epoch,
                    FLAGS.num_epochs,batch_idx,len(loader)))
            if train:
                print(random_list)
            for width_mult in sorted(FLAGS.width_mult_list_test, reverse=True):
                if train and width_mult not in [max_width, min_width]:
                    continue
                results = scalar_meters(meters[str(width_mult)])
                logging.info('{}\t{}\tepoch:{}/{}\t{}/{} '.format(
                    phase, str(width_mult), epoch,
                    FLAGS.num_epochs,batch_idx,len(loader)) +
                ', '.join('{}: {:.3f}'.format(k, v) for k, v in results.items()))

        # elif is_master() and batch_idx % FLAGS.print_freq ==0:
        #     results = flush_scalar_meters(meters)
        #     print('Time {:.3f}s ({:.3f})s\tData {:.3f}s ({:.3f})s\t{}\t{}\tepoch:{}/{}\t{}/{} '.format(
        #             batch_time.val, batch_time.avg, data_time.val,data_time.avg,phase, str(width_mult), epoch,
        #             FLAGS.num_epochs,batch_idx,len(loader)) + ', '.join(
    ...
    if is_master() and getattr(FLAGS, 'slimmable_training', False) :
        for width_mult in sorted(FLAGS.width_mult_list_test, reverse=True):
            ...
            logging.info('{}\t{}\tepoch:{}/{}\t{}/{} '.format(
                    ...
                    FLAGS.num_epochs,batch_idx,len(loader)) +
                ', '.join('{}: {:.3f}'.format(k, v) for k, v in results.items()))
            results_lst[str(width_mult)] = results
        
    return results_lst


def train_val_test_SuperNet():
    """train and val"""
    ...
    # args.save = sub_remark

    generate_date = str(datetime.now().date())

    path = os.path.join(generate_date, sub_remark + "."+str(FLAGS.search_space))
    if is_master():
        # logging.basicConfig(level=logging.WARNING,
        #             filename='./logs/log.txt',
        #             filemode='w',
        #             format='%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s')
        create_exp_dir(generate_date, path)
        logging = get_logger(path +'/log')
    ...
    #     else:
    #         FLAGS.width_mult_list = FLAGS.width_mult_list_test
    ...
    for epoch in range(last_epoch+1, FLAGS.num_epochs+1):
        ...
        print("\n")
        logging.info("\nEpoch {}".format(epoch))
        logging.info("Search Space: {}".format(FLAGS.search_space))
        logging.info("epoch time {}".format(time.time()-end_epoch))
        end_epoch = time.time()

        lr_scheduler.step()
        # train
        results_lst = run_one_epoch(
            epoch, train_loader, model_wrapper, criterion, optimizer,
            train_meters, logging, phase='train', soft_criterion=soft_criterion)
        ...
        #         },
        #         os.path.join(FLAGS.log_dir, 'US-Net_best_model.pt'))
        ...
        if is_master() :
            ...
            idx = 0
            for width_mult in sorted(FLAGS.width_mult_list_test, reverse=True):
                error_mtx[idx].append(results_lst[str(width_mult)]['top1_error'])
                idx = idx + 1

            logging.info(error_mtx)
        ...
        if is_master() and epoch % 10 == 0:
            ...
            path1 = os.path.join(path, "checkpoint")
            if not os.path.isdir(path1):
                os.mkdir(path1)
            torch.save(checkpoint, path + str('/checkpoint/epoch_%s.pth' %(str(epoch))))




def run_one_epoch_ChildNet(
        epoch, loader, model, criterion, optimizer, meters, logging, phase='train',
        soft_criterion=None):
    """run one epoch for train/val/test/cal"""
    t_start = time.time()
    ...
    for batch_idx, (input, target) in enumerate(loader):
            ...
            if getattr(FLAGS, 'slimmable_training', False):
                if getattr(FLAGS, 'universally_slimmable_training', False):
                    ...
                    for width_mult in sorted(FLAGS.width_mult_list_test, reverse=True):
                        ...
                        f_model = open(model_pool, 'r')
                        blocksize_list = eval(f_model.readlines()[FLAGS.model_idx].strip())
        ...
        if is_master() and getattr(FLAGS, 'slimmable_training', False) and batch_idx % FLAGS.print_freq ==0:
            if batch_idx % (FLAGS.print_freq * 5) == 0:
                print('\nTime {:.3f}s ({:.3f})s\tData {:.3f}s ({:.3f})s\t{}\t{}-{}\tEpoch:{}/{}\t{}/{} '.format(
                        batch_time.val, batch_time.avg, data_time.val,data_time.avg,phase, FLAGS.search_space , FLAGS.model_idx, epoch,
                        FLAGS.num_epochs,batch_idx,len(loader)))

            for width_mult in sorted(FLAGS.width_mult_list_test, reverse=True):
                results = scalar_meters(meters[str(width_mult)])
                logging.info('{}\tModel idx: {}-{}\tEpoch:{}/{}\t{}/{} '.format(
                    phase, FLAGS.search_space , FLAGS.model_idx, epoch,
                    FLAGS.num_epochs,batch_idx,len(loader)) +
                ', '.join('{}: {:.3f}'.format(k, v) for k, v in results.items()))
...
def train_val_test_ChildNet():
    ...
    path = os.path.join(generate_date, sub_remark + "."+str(FLAGS.search_space)+"-"+str(FLAGS.model_idx))
    if is_master():
        # logging.basicConfig(level=logging.WARNING,
        #             filename='./logs/log.txt',
        #             filemode='w',
    ...
    #         FLAGS.width_mult_list = FLAGS.width_mult_list_test
    temp = []
    temp.append(FLAGS.model_idx)
    FLAGS.width_mult_list_test = temp
    ...
    for epoch in range(last_epoch+1, FLAGS.num_epochs+1):
        ...
        lr_scheduler.step()
        # train
        results_lst = run_one_epoch_ChildNet(
            epoch, train_loader, model_wrapper, criterion, optimizer,
            train_meters, logging, phase='train', soft_criterion=soft_criterion)
        ...
        if is_master() :
            # logging.info(results)
            for width_mult in sorted(FLAGS.width_mult_list_test, reverse=True):
                error_mtx.append(results_lst[str(width_mult)]['top1_error'])
...
def slimming(loader, model, criterion):
    ...
    while True:
        ...
        print(['{element:.4f}' for element in error])
...
def main():
    ...
    # train_val_test_SuperNet()

    if getattr(FLAGS, 'train_alone_tag', False):
        train_val_test_ChildNet()
    else:
        train_val_test_SuperNet()
