...
def run_one_epoch(
    ...
    if is_master() and batch_idx % FLAGS.print_freq ==0:
        results = flush_scalar_meters(meters)
...
def train_val_test():
    ...
    path = os.path.join('logs',generate_date, sub_remark)
    if is_master():
        # logging.basicConfig(level=logging.WARNING,
        #             filename='./logs/log.txt',
        #             filemode='w',
        #             format='%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s')
        logging = get_logger('./logs/S-Net.log.txt')
    ...
    for epoch in range(last_epoch+1, FLAGS.num_epochs):
        ...
        if is_master() :
            ...
            error_lst.append(results['top1_error'])
            logging.info(error_lst)
